...
static asciinode * build_ascii_tree_recursive(Noeud * t)
{
    asciinode * node;
    ...
    node->left = build_ascii_tree_recursive(t->filsGauche);
    node->right = build_ascii_tree_recursive(t->filsDroit);
...
static  asciinode * build_ascii_tree(Collection * t)
{
    ...
    node = build_ascii_tree_recursive(avl->racine);
#else
    // TODO
#endif
...
void afficherArbreEnAscii(Collection * t)
{
    asciinode *proot;
    int xmin, i;
    if (t == NULL) return;
